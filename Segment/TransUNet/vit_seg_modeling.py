...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, checkpoint_path=None,pretrained=False,ours=None, zero_head=False, vis=False):
        super(VisionTransformer, self).__init__()
        # self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = 'seg'
        self.transformer = Transformer(img_size, vis,checkpoint_path,pretrained,ours)
        self.decoder = DecoderCup()
        self.segmentation_head = SegmentationHead(
            in_channels=(256, 128, 64, 16)[-1],
            out_channels=1,
            kernel_size=3,
        )
        # self.config = config
