...
from Detection.Faster_R_CNN_FPN_512 import get_model
...
if __name__ == "__main__":
    ...
    #     hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model_512_new_new/{hypar['dataset']}/efficientnet-b2-mammo-clip" ## model weights saving (or restoring) path
    ...
    # if hypar["mode"] == "train":
    #     hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
    #     hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model_512_new_new/{hypar['dataset']}/efficientnet-b5-mammo-clip" ## model weights saving (or restoring) path
    #     hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
    #     hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
    #     hypar["plot_output"]=False
    # else:
    ...
    # if hypar["mode"] == "train":
    #     hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
    #     hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model_512_new_new3/{hypar['dataset']}/efficientnet-ours" ## model weights saving (or restoring) path
    #     hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
    #     hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
    #     hypar["plot_output"]=False
    # else:
    ...
    # if hypar["mode"] == "train":
    #     hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
    #     hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model_512_new_new/{hypar['dataset']}/lvmmed-resnet50" ## model weights saving (or restoring) path
    #     hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
    #     hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
    #     hypar["plot_output"]=False
    # else:
    ...
    # if hypar["mode"] == "train":
    #     hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
    #     hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model_512_new_new3/{hypar['dataset']}/lvmmed" ## model weights saving (or restoring) path
    #     hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
    #     hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
    #     hypar["plot_output"]=False
    # else:
    ...
    # if hypar["mode"] == "train":
    #     hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
    #     hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model_512_new_new/{hypar['dataset']}/medsam" ## model weights saving (or restoring) path
    #     hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
    #     hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
    #     hypar["plot_output"]=False
    # else:
    ...
    # if hypar["mode"] == "train":
    #     hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
    #     hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model_ft/{hypar['dataset']}/ours-mammo1" ## model weights saving (or restoring) path
    #     hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
    #     hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
    #     hypar["plot_output"]=False
    # else:
    ...
    # if hypar["mode"] == "train":
    #     hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
    #     hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model/{hypar['dataset']}/RaSE" ## model weights saving (or restoring) path
    #     hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
    #     hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
    #     hypar["plot_output"]=False
    # else:
    #     hypar["valid_out_dir"] = f"{os.path.dirname(current_dir)}/Detection/results/{hypar['dataset']}/RaSE"##"../DIS5K-Results-test" ## output inferenced segmentation maps into this fold
    #     hypar["valid_out_bbox_dir"] = f"{os.path.dirname(current_dir)}/Detection/bbox_results/{hypar['dataset']}/RaSE"
    #     hypar["restore_model"] = ""
    # hypar["model"]=get_model(backbone_name="RaSE")
    # main(hypar=hypar)
    
    # # # #RaSE
    # if hypar["mode"] == "train":
    #     hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
    #     hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model/{hypar['dataset']}/RaSE_random" ## model weights saving (or restoring) path
    #     hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
    #     hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
    #     hypar["plot_output"]=False
    # else:
    #     hypar["valid_out_dir"] = f"{os.path.dirname(current_dir)}/Detection/results/{hypar['dataset']}/RaSE_random"##"../DIS5K-Results-test" ## output inferenced segmentation maps into this fold
    #     hypar["valid_out_bbox_dir"] = f"{os.path.dirname(current_dir)}/Detection/bbox_results/{hypar['dataset']}/RaSE_random"
    #     hypar["restore_model"] = ""
    # hypar["model"]=get_model(backbone_name="RaSE",pretrained=False)
    # main(hypar=hypar)
    
    # # # #RaSE_Swin
    if hypar["mode"] == "train":
        hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
        hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model/{hypar['dataset']}/RaSE_Swin" ## model weights saving (or restoring) path
        hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
        hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
        hypar["plot_output"]=False
    else:
        hypar["valid_out_dir"] = f"{os.path.dirname(current_dir)}/Detection/results/{hypar['dataset']}/RaSE_Swin"##"../DIS5K-Results-test" ## output inferenced segmentation maps into this fold
        hypar["valid_out_bbox_dir"] = f"{os.path.dirname(current_dir)}/Detection/bbox_results/{hypar['dataset']}/RaSE_Swin"
        hypar["restore_model"] = ""
    hypar["model"]=get_model(backbone_name="RaSE_Swin",pretrained=True)
    main(hypar=hypar)
    
     # # # #RaSE_Swin_random
    if hypar["mode"] == "train":
        hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
        hypar["model_path"] =f"{os.path.dirname(current_dir)}/Detection/saved_model/{hypar['dataset']}/RaSE_Swin_random" ## model weights saving (or restoring) path
        hypar["restore_model"] = "" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
        hypar["start_ite"] = 0 ## start iteration for the training, can be changed to match the restored training process
        hypar["plot_output"]=False
    else:
        hypar["valid_out_dir"] = f"{os.path.dirname(current_dir)}/Detection/results/{hypar['dataset']}/RaSE_Swin_random"##"../DIS5K-Results-test" ## output inferenced segmentation maps into this fold
        hypar["valid_out_bbox_dir"] = f"{os.path.dirname(current_dir)}/Detection/bbox_results/{hypar['dataset']}/RaSE_Swin_random"
        hypar["restore_model"] = ""
    hypar["model"]=get_model(backbone_name="RaSE_Swin",pretrained=False)
    main(hypar=hypar)